...
def main(params):
    ...
    if params['split'] == 1:
        print("gayatri - printing and loading anootations and questions")
        with open('annotations/Annotations_Train_mscoco/mscoco_train2014_annotations.json') as file_object:
            train_anno = json.load(file_object)
        val_anno=json.load(open('annotations/Annotations_Val_mscoco/mscoco_val2014_annotations.json', 'r'))
        train_ques = json.load(open('annotations/Questions_Train_mscoco/MultipleChoice_mscoco_train2014_questions.json', 'r'))
        val_ques = json.load(open('annotations/Questions_Val_mscoco/MultipleChoice_mscoco_val2014_questions.json', 'r'))
        subtype = 'train2014'
        ...
        train_anno = json.load(open('annotations/Annotations_Train_mscoco/mscoco_train2014_annotations.json', 'r'))
        val_anno = json.load(open('annotations/Annotations_Val_mscoco/mscoco_val2014_annotations.json', 'r'))

        train_ques = json.load(open('annotations/Questions_Train_mscoco/MultipleChoice_mscoco_train2014_questions.json', 'r'))
        val_ques = json.load(open('annotations/Questions_Val_mscoco/MultipleChoice_mscoco_val2014_questions.json', 'r'))
        test_ques = json.load(open('annotations/Questions_Test_mscoco/MultipleChoice_mscoco_test2015_questions.json', 'r'))
